...
const defaultFooter: FooterSettings = {
  ...
  facebook: "",
  twitter: "",
  brand: emptyTranslation(),
  description: emptyTranslation(),
  quickLinks: emptyTranslation(),
  contactInfo: emptyTranslation(),
  home: emptyTranslation(),
  products: emptyTranslation(),
  reviews: emptyTranslation(),
  favorites: emptyTranslation(),
  cart: emptyTranslation(),
  rights: emptyTranslation(),
  links: [],
};
...
function FooterTranslationField({
  label,
  value,
  onChangeEn,
  onChangeAr,
}: FooterTranslationFieldProps) {
  return (
    <div className="md:col-span-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <input
          ...
          placeholder="English"
          className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#c5a059]/50"
        />
        <input
          value={value.ar ?? ""}
...
function FooterContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          const message =
            e instanceof Error ? e.message : "Could not load footer settings.";
    ...
    // Load once on mount only — do not add toast/footer deps or inputs will reset
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const message =
        e instanceof Error ? e.message : "Could not save footer settings.";
  ...
  const setTranslation = useCallback(
    (field: keyof FooterSettings, lang: "en" | "ar", value: string) => {
      setFooter((prev) => ({
        ...prev,
        [field]: {
          ...((prev[field] as FooterTranslation) || emptyTranslation()),
          [lang]: value,
        },
      }));
    },
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-5 lg:p-8 max-w-5xl">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-2xl font-black">Footer Settings</h1>
              <p className="text-gray-500 text-sm mt-1">
                Manage footer content shown on the website
              </p>
            </div>
            <button
              onClick={handleSave}
          ...
          <div className="space-y-6">
            <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6">
              <h2 className="font-black text-sm uppercase tracking-widest text-[#c5a059] mb-5">
                Contact Details
              </h2>
            ...
            <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <FooterField
                  label="Hours (Arabic)"
                  icon={FaClock}
                  value={footer.hoursAr ?? ""}
                  onChange={(v) => setField("hoursAr", v)}
                  dir="rtl"
                />
              </div>
            </div>
            ...
            <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6">
              ...
              <div className="grid grid-cols-1 gap-5">
                <FooterTranslationField
                  label="Brand Name"
                  value={footer.brand ?? emptyTranslation()}
                  onChangeEn={(v) => setTranslation("brand", "en", v)}
                  onChangeAr={(v) => setTranslation("brand", "ar", v)}
                />
                <FooterTranslationField
                  label="Description"
                  value={footer.description ?? emptyTranslation()}
                  onChangeEn={(v) => setTranslation("description", "en", v)}
                  onChangeAr={(v) => setTranslation("description", "ar", v)}
                />
                <FooterTranslationField
                  label="Quick Links Heading"
                  value={footer.quickLinks ?? emptyTranslation()}
                  onChangeEn={(v) => setTranslation("quickLinks", "en", v)}
                  onChangeAr={(v) => setTranslation("quickLinks", "ar", v)}
                />
                <FooterTranslationField
                  label="Contact Info Heading"
                  value={footer.contactInfo ?? emptyTranslation()}
                  onChangeEn={(v) => setTranslation("contactInfo", "en", v)}
                  onChangeAr={(v) => setTranslation("contactInfo", "ar", v)}
                />
                <FooterTranslationField
                  label="Copyright"
                  value={footer.rights ?? emptyTranslation()}
                  onChangeEn={(v) => setTranslation("rights", "en", v)}
                  onChangeAr={(v) => setTranslation("rights", "ar", v)}
                />
              </div>
            </div>
            ...
            <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-black text-sm uppercase tracking-widest text-[#c5a059]">
                  Quick Links
                </h2>
                <button
                  type="button"
                  onClick={addLink}
                  className="flex items-center gap-2 text-sm font-bold text-[#c5a059] hover:text-[#e6c35f]"
                >
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <input
                        value={link.labelAr}
                        onChange={(e) =>
                          updateLink(index, { labelAr: e.target.value })
                        }
