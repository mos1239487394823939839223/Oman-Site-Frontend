...
function getBannerImageSrc(banner: Banner): string {
  ...
  return resolveMediaUrl(raw, 'banners');
}
...
export default function Slider() {
  ...
  if (loading) {
    return (
      <div className="relative w-full h-[260px] sm:h-[360px] md:h-[500px] bg-white/5 animate-pulse rounded-[2.25rem] sm:rounded-[3rem] overflow-hidden border border-white/10" />
    );
  }
  ...
  return (
    <div className="relative w-full h-[320px] sm:h-[420px] md:h-[520px] lg:h-[600px] overflow-hidden rounded-[2.25rem] sm:rounded-[3rem] shadow-2xl group border border-white/5">
      <div className="flex h-full flex-col lg:flex-row">
        {/* Main Featured Slide */}
        <div className="flex-[2] relative overflow-hidden">
          <Image
            src={banners[currentSlide] ? getBannerImageSrc(banners[currentSlide]) : "/placeholder.svg"}
          ...
          {[1, 2].map((offset) => {
            ...
            return (
              <div key={banner?._id || offset} className="flex-1 relative rounded-[2rem] overflow-hidden group/side">
                <Image
                  src={banner ? getBannerImageSrc(banner) : "/placeholder.svg"}
                ...
                <div className="absolute bottom-6 left-6 right-6 text-white">
                  <h3 className="text-xl font-black text-white drop-shadow-md mb-3 line-clamp-1" style={{ color: 'white' }}>{banner?.name}</h3>
                  <Link 
